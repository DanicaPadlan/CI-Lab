...
void advance_lexer(void) {
    //if next token is end of line token
    if (next_token->ttype == TOK_EOL) {
        //handle error and return nothing
        handle_error(ERR_SYNTAX);
        return;
    }

    //sets this token pointer temp to hold this_token?
    lptr_t tmp = this_token;
    //this token gets switched to next token
    this_token = next_token;
    //next token gets switched to old this_token
    next_token = tmp;
    //finds next token of old this_token?
    get_token(next_token);
}
